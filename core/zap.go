...
func Zap() (logger *zap.Logger) {
	if ok, _ := utils.PathExists(global.MARKET_CONFIG.Zap.Director); !ok { // 判断是否有Director文件夹
...
func getEncoderConfig() (config zapcore.EncoderConfig) {
	...
	switch {
	case global.MARKET_CONFIG.Zap.EncodeLevel == "LowercaseLevelEncoder": // 小写编码器(默认)
		config.EncodeLevel = zapcore.LowercaseLevelEncoder
	case global.MARKET_CONFIG.Zap.EncodeLevel == "LowercaseColorLevelEncoder": // 小写编码器带颜色
		config.EncodeLevel = zapcore.LowercaseColorLevelEncoder
	case global.MARKET_CONFIG.Zap.EncodeLevel == "CapitalLevelEncoder": // 大写编码器
		config.EncodeLevel = zapcore.CapitalLevelEncoder
	case global.MARKET_CONFIG.Zap.EncodeLevel == "CapitalColorLevelEncoder": // 大写编码器带颜色
...
func CustomTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format(global.MARKET_CONFIG.Zap.Prefix + "2006/01/02 - 15:04:05.000"))
}
